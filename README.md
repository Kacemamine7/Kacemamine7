<h1 align="center">Hi there! I'm [Kacem amine ELhajji\الحاجي قاسم امين] 👋</h1>

<h3 align="center">High School Student & Aspiring Computer Scientist</h3>

<br />

<p align="center">
    <a href="mailto:[kacemamine.elhajji@gmail.com]">
        <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white" height="40" />
    </a>
    <a href="(https://www.linkedin.com/in/kacem-amine-elhajji-a24a9938b/)">
        <img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white" height="40" />
    </a>
    <a href="[Your Portfolio URL (optional)]">
        <img src="https://img.shields.io/badge/Portfolio-5E24B0?style=for-the-badge&logo=ko-fi&logoColor=white" height="40" />
    </a>
</p>

---

## 💻 About Me

I'm a high school student currently diving into the world of computer science through **Harvard's CS50x: Introduction to Computer Science**. I'm passionate about problem-solving and excited to explore how code can build impactful applications.

My main focus right now is mastering the fundamentals, from **C** and **Python** to **web development**.

<a href="https://cs50.harvard.edu/x/2024/" target="_blank"><img align="right" src="https://upload.wikimedia.org/wikipedia/commons/e/ee/Harvard_shield_with_slogan.svg" width="150" height="150" alt="<img width="256" height="256" alt="image" src="https://github.com/user-attachments/assets/e572468b-93bd-4fbd-985c-03411e6f243f" />
" /></a>

## 🚀 My CS50 Journey

This repository tracks my progress through the weekly problem sets and final project of CS50x.

| Week | Topic | Languages/Concepts Covered | Status |
| :--- | :--- | :--- | :--- |
| **0** | Scratch | Logic, Abstraction | ✅ Completed |
| **1** | C | Basic Syntax, Functions, Loops | ✅ Completed |
| **2** | Arrays | Command-Line Arguments, Compiling | ⏳ In Progress |
| **3** | Algorithms | Searching, Sorting, Big O Notation | 📚 Planning |
| **...** | (More weeks to be added) | | |

## 🛠️ Languages and Tools I'm Learning

Here are the technologies and languages I'm actively working with in the course:

<p>
    <img src="https://img.shields.io/badge/C-00599C?style=for-the-badge&logo=c&logoColor=white" alt="C" />
    <img src="https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white" alt="Python" />
    <img src="https://img.shields.io/badge/HTML5-E34F26?style=for-the-badge&logo=html5&logoColor=white" alt="HTML5" />
    <img src="https://img.shields.io/badge/CSS3-1572B6?style=for-the-badge&logo=css3&logoColor=white" alt="CSS3" />
    <img src="https://img.shields.io/badge/SQL-4479A1?style=for-the-badge&logo=mysql&logoColor=white" alt="SQL" />
    <img src="https://img.shields.io/badge/Visual_Studio_Code-0078D4?style=for-the-badge&logo=visual-studio-code&logoColor=white" alt="VS Code" />
</p>

## 💡 Key CS50 Projects

Here's a quick look at some of the key problem sets and projects I've tackled:

* **PSET 1: Mario (Less/More):** Recreated the half-pyramid from the Super Mario Bros. game using nested loops in C.
* **PSET 2: Readability:** Calculated the reading level of a text based on the Coleman-Liau index.
* **Final Project:** *[To be determined—maybe a web app using Flask!]*

---

<h4 align="center">
    "This is CS50."
    <br/>
    <a href="https://cs50.harvard.edu/x/2024/">Start your journey!</a>
</h4>
<img align="right" src="https://github.com/blackcater/blackcater/raw/main/images/banner.gif" height="200" style="max-width: 100%; height: auto; max-height: 200px; display: inline-block;" data--h-bstatus="0OBSERVED" class="hb-blur-temp" data-target="animated-image.originalImage">
